...
public class ViewAllRoomsView implements View {
    ...
    private void refresh() {
        CompletableFuture.supplyAsync(
                () -> hotelManager.getRoomService().getAllRooms(),
                BackgroundTaskService.getExecutor()
        ).thenAcceptAsync(rooms -> roomData.setAll(rooms), Platform::runLater)
         .exceptionally(ex -> {
             LOGGER.log(Level.WARNING, "Failed to refresh room list", ex);
             return null;
         });
    }
}